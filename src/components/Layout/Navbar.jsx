...
import { chainIdLocalStorageKey, connectorLocalStorageKey, SUPPORTED_CHAIN_IDS, truncateWalletString } from "../../utils";
import { connectors } from "../../utils/web3React";
import { setLocalStorageByUserinfo } from '../../utils'

function Navbar(props) {
  const { navbar, setNavbar, showDropdown, dropDownChange, createNftOption, selectNftCreateOption, showSignUpModal, t,
    select, onSelectCountry, showWalletConnectModal, setShowWalletConnectModal, setSignUpShowModal, openCreateNftModal, closeModals, continueCreateNft, setShowDropdown
  } = props
  const { loginStatus, chainId, account, library } = useContext(Web3WalletContext)

  const dispatch = useDispatch();
  const { login, logout, switchNetwork } = useAuth();
  ...
  /**
   * Connet wallet function with connectorId
   * @param {*} connectorId 
   */
  const connectWallet = async (connectorId) => {
    try {
      if (loginStatus && userInfo?.isApproved) {
  ...
  /**
   * When clicked disconnect button 
   */
  const disconnect = () => {
    ...
    setLocalStorageByUserinfo({ ...user })
  ...
  /**
   * When clicked switch network button 
   */
  const switchBtn = () => {
    setShowWalletOptions(false)
  ...
  return (
    <>
      <nav className="w-full h-20 border fixed bg-whtie top-0 z-[40]">
        <div className="justify-between px-4 md:items-center md:flex md:px-8 bg-white">
          <div>
          ...
          <div className='w-3/4'>
            ...
            >
              <div className='bg-[#F6F6F6] h-10 w-[400px] md:w-[380px] rounded-lg py-[10px] px-4 text-center'>
                <i className="fa fa-search float-left text-[#8b95a7] mt-1" aria-hidden="true"></i>
                <input placeholder='Search items, colletions' className='bg-transparent border-0 focus:outline-none font-[500] w-5/6' />
                <img src='/imgs/shortcut.png' className='float-right mt-[2px]' alt='A' />
              </div>
              <div className='flex'>
                <div className='flex'>
                  ...
                  <ReactFlagsSelect
                    ...
                    className='h-10 mx-2 country-selector font-[500]'
                  ...
                  />
                  {
                    loginStatus && userInfo?.isApproved
                      ?
                      <div className='bg-[#F6F6F6] flex pxy-2 px-3 rounded-lg w-64 justify-between mx-2 mr-10 h-10'>
                        <img src='/imgs/metamask-icon.png' alt='' />
                        <p className='ml-2 text-[#6E7B91] w-fit py-2'>{loginStatus && account ? `${truncateWalletString(account)}` : ''}</p>
                        <i className={`fa pt-3 ml-2 fa-chevron-down text-slate-500 font-[500]`} aria-hidden="true" role="button" onClick={() => { if (showWalletOptions || showSwitchWallet) { setShowWalletOptions(false); setShowSwitchWallet(false) } else setShowWalletOptions(true) }}></i>
                        {(showWalletOptions || showSwitchWallet) && <div className="bg-translate absolute w-full h-[2000px] top-0 left-0 z-15" onClick={() => { setShowWalletOptions(false); setShowSwitchWallet(false) }}></div>}
                        {showWalletOptions && <div className="absolute mt-12 -ml-6 shadow-2xl rounded-xl z-20">
                          <div className="rounded-xl bg-white p-2 border w-64">
                            <button className='bg-[#6823D0] rounded-lg text-white text-center my-1 py-2 px-4 w-full font-bold' onClick={disconnect}>
                              {t('Disconnect Wallet')}
                            </button>
                        ...
                        {showSwitchWallet &&
                          <div className="absolute mt-12 -ml-6 shadow-2xl rounded-xl">
                            <div className="rounded-xl bg-white p-2 border w-64">
                              <p className="border-b text-center py-2 px-3 text-slate-500 font-semibold text-xl">Select a network</p>
                      ...
                      <button className='bg-[#6823D0] rounded-lg text-white text-center py-2 px-4 mr-2' onClick={connectBtn}>
                        {loginStatus && userInfo?.isApproved ? t('Disconnect Wallet') : t('Connect Wallet')}
                      </button>
                  }
                  ...
                  <div className='inline-flex'>
                    <img src='/imgs/avatar-1.png' className='w-8 h-8' />
                    ...
                    <div className={`bg-white shadow border rounded-lg p-2 absolute top-16 right-6 w-[220px] z-20 ${showDropdown ? '' : 'hidden'}`}>
                    ...
                    {showDropdown && <div className="bg-translate absolute w-full h-[2000px] top-0 left-0 z-10" onClick={() => setShowDropdown(false)}></div>}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div id="crypto-modal" tabIndex="-1" aria-hidden="true" className={`${showSignUpModal ? '' : 'hidden'} mx-auto fixed mt-16 sm:mt-32 z-50 w-fit p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-fit`}>
          <div className="relative w-full h-full max-w-md md:h-auto">
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 sm:px-10 sm:py-5 p-2">
              <button type="button" onClick={() => setSignUpShowModal(false)} className="text-lg absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="crypto-modal">
              ...
              <div className="p-6">
                <p className="font-normal text-[#6E7B91] dark:text-gray-400">Choose how you want to connect. There are several wallet providers.</p>
      ...
      <div id="create-nft-modal" tabIndex="-1" aria-hidden="true" className={`${openCreateNftModal ? '' : 'hidden'} mx-auto absolute mt-16 sm:mt-32 z-[55] w-full md:w-[800px] p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-fit`}>
        <div className="relative w-full h-full md:h-auto p-4">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 sm:px-6 sm:py-10 p-2 w-full text-center">
            <button type="button" onClick={closeModals} className="text-lg absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="crypto-modal">
              <i className='fa fa-times-circle-o'></i>
              <span className="sr-only">Close modal</span>
            </button>
            <div className="px-6 py-4 rounded-t dark:border-gray-600">
            ...
            <p className="font-[500] text-[#6E7B91] dark:text-gray-400 text-center">Choose how you want to connect. There are several wallet providers.</p>
            <div className='flex my-4 md:my-10' >
              <div className={`w-1/2 mx-2 rounded-lg hover:border-[#6823D0] border p-2 md:p-8 text-center justify-center ${createNftOption === 'single' ? 'border-[#6823D0]' : ''}`} role='button' onClick={() => selectNftCreateOption('single')}>
                ...
                <p className="font-[500] text-sm text-[#6E7B91] dark:text-gray-400 text-center my-2">If you want to highlight the uniqueness and individuality of your team.</p>
                <input type="radio" checked={createNftOption === 'single'} readOnly />
              </div>

              <div className={`w-1/2 mx-2 rounded-lg hover:border-[#6823D0] border p-2 md:p-8 text-center justify-center ${createNftOption === 'multi' ? 'border-[#6823D0]' : ''} `} role='button' onClick={() => selectNftCreateOption('multi')}>
                <img src='/imgs/vector-1.png' className='mx-auto my-2' alt='' />
                <h3 className="text-base text-center font-bold text-[#000549] lg:text-lg dark:text-white my-2" >
                  Multiple
                </h3>
                <p className="font-[500] text-[#6E7B91] dark:text-gray-400 text-center my-2 text-sm">If you want to share your NFT with a large number of community members</p>
                <input type="radio" checked={createNftOption === 'multi'} readOnly />
              </div>
            </div>
      ...
      <div id="wallet-connect-modal" tabIndex="-1" aria-hidden="true" className={`${showWalletConnectModal ? '' : 'hidden'} mx-auto fixed mt-16 sm:mt-32 z-50 w-full md:w-[800px] p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-fit`}>
        <div className="relative w-full h-full md:h-auto p-4">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 sm:px-8 sm:py-20 p-2 w-full text-center">
            ...
            <p className="font-normal text-[#6E7B91] dark:text-gray-400 text-center">Choose how you want to connect. There are several wallet providers.</p>
            <div className='my-4 mx-auto md:my-10 border w-4/5 rounded-lg'>
              {connectors.map((connector) => (
                <div
                  key={connector.connectorId}
                  onClick={() => connectWallet(connector.connectorId)}
                  className="p-6 flex border-b w-full justify-between cursor-pointer"
                >
                  <div className="flex">
                    <img src={`/imgs/${connector.title}.png`} className='w-8' />
                    <p className="text-[#303C4F] font-semibold ml-4 text-lg">
                      {connector.title}
                    </p>
                  </div>
                  {connector.title === 'Metamask' && <button className="float-right px-3 py-1 rounded-xl bg-[#1CB23C] text-white">
                    Popular
                  </button>}
                </div>
              ))}
