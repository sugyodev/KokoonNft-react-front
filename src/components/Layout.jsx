...
const providerOptions = {
  walletconnect: {
    ...
    options: {
      infuraId: '9dd88b07c58d4af3998955cf0808a9e9',
  ...
  "custom-example": {
    ...
    options: {
      apiKey: "9dd88b07c58d4af3998955cf0808a9e9"
    },
...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    div.id = 'added-elem'
    div.innerHTML = "<h1 style='color:#000549;font-size:28px;font-weight:700'>Connect Wallet</h1><p style='color:#6E7B91;margin-top:10px;margin-bottom:40px'>Choose how you want to connect. There are several wallet providers.</p>"
    btn.innerHTML = "<button style='color:white;background:green; padding:2px 8px;border-radius:16px'>Popular<button>"
    let modal = document.getElementsByClassName('sc-furwcr')[0];
    if (!document.getElementById('added-elem')) {
      modal.insertBefore(div, modal.childNodes[0]);
      modal.childNodes[1].children[0].append(btn)
    }
  }, [])
  ...
  let initPaths = ['/signin', '/joinin', '/signup', '/select-package', '/payment-page', '/payment-success', '/create-new-nft', '/choose-chain'
    , '/create-new-nft/solana', '/create-new-nft/ethereum', '/create-new-nft/tezos', '/create-new-nft/polygon']
    ...
    <>
      <React.Fragment>
        <div className='bg-[#FAFAFA] h-fit'>
          ...
          {isInitPath &&
            <>
              <div className={`${openCreateNftModal ? '' : 'hidden'} w-full top-0 opacity-50 h-[110%] bg-zinc-700 absolute z-50`} onClick={() => closeCreateNftModal(false)}></div>
                        ...
                        {walletConnected
                          ? <div className='flex'>
                            ...
                            <ReactFlagsSelect
                              ...
                              selectedSize={14}
                            />
                            <div className='bg-[#F6F6F6] flex pxy-2 px-3 rounded-lg w-44 justify-between mr-2 h-10'>
                          ...
                          : <button className='bg-[#6823D0] rounded-lg text-white text-center py-2 px-4' onClick={() => { onConnectBtn() }}>
                            {t('Connect Wallet')}
                          </button>}
                      </div>
                    </div>
                    ...
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 sm:px-10 sm:py-5 p-2">
                      ...
                      <div className="p-6">
                        ...
                        <ul className="my-4 space-y-1">
                          ...
                          <li onClick={() => connectWallet('coinbase')}>
                            <a href="#" className="flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
                          ...
                          <li onClick={() => connectWallet('phantom')}>
                            <a href="#" className="flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
                ...
                <div id="create-nft-modal" tabIndex="-1" aria-hidden="true" className={`${openCreateNftModal ? '' : 'hidden'} mx-auto fixed mt-16 sm:mt-32 z-50 w-full md:w-[800px] p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-fit`}>
                  <div className="relative w-full h-full md:h-auto p-4">
                    <div className="relative bg-white rounded-lg shadow dark:bg-gray-700 sm:px-10 sm:py-10 p-2 w-full text-center">
                      <button type="button" onClick={closeCreateNftModal} className="text-lg absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="crypto-modal">
                        <i className='fa fa-times-circle-o'></i>
                        <span className="sr-only">Close modal</span>
                      </button>
                      <div className="px-6 py-4 rounded-t dark:border-gray-600">
          ...
          {/* Sidbar Component */}
          {isInitPath && <div className={`w-[300px] h-[calc(100%)] border-t-0 border absolute left-0 top-20 p-6 sm:block z-[2] bg-white sidebar transition-all duration-300 shrink-0 ${navbar ? "block" : "hidden"}`}>
